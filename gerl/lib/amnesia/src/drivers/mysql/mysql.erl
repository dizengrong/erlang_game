...
%% Function: stop(Id)
%%           Id      = term(), connection-group Id
%% Descrip.: Terminate a connection.
%% Returns : ok
%%--------------------------------------------------------------------
stop(Id) ->
...
%%--------------------------------------------------------------------
%% Function: handle_call({stop, Id}, From, State)
%%           Id    = term(), connection-group id
%% Descrip.: Stop the connection
%% Returns : {reply, ok, State}
%%--------------------------------------------------------------------
handle_call({stop, Id}, _From, State) ->
    log(State#state.log_fun, debug, "mysql: stop (id ~p)", [Id]),
    case remove_mysql_connection_using_id(Id, State#state.conn_list, []) of
	{ok, Conn, NewConnList} ->
            Conn#mysql_connection.conn_pid ! {stop},
	    {stop, normal, ok, State#state{conn_list = NewConnList}};
	nomatch ->
	    %% we have no active connection matching Id
	    {reply, {error, no_connection}, State}
    end;
...
terminate(Reason, State) ->
    ...
    log(LogFun, LogLevel, "mysql: Terminating with reason : ~p~n", [Reason]),
    Reason.
...
%%--------------------------------------------------------------------
%% Function: remove_mysql_connection_using_id(Id, ConnList)
%%           ID        = term()
%%           ConnList = list() of mysql_connection record()
%% Descrip.: Removes the first mysql_connection in ConnList that has
%%           a pid matching Pid.
...
%%           NewConnList = list() of mysql_connection record()
%%--------------------------------------------------------------------
remove_mysql_connection_using_id(Id, [#mysql_connection{id = Id} = H | T], Res) ->
    {ok, H, lists:reverse(Res) ++ T};
remove_mysql_connection_using_id(Id, [H | T], Res) when is_record(H, mysql_connection) ->
    remove_mysql_connection_using_pid(Id, T, [H | Res]);
remove_mysql_connection_using_id(_Id, [], _Res) ->
    nomatch.
